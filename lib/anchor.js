  ...
  generateServerRoutes(endpoints) {
    ...
    endpoints.forEach(ep => {
      ...
      if (ep.requiredFields) {
        ...
        code += `    const missing = [${ep.requiredFields.map(f => `'${f}'`).join(', ')}].filter(f => !req.body[f]);\n`;
        code += `    if (missing.length > 0) return res.status(400).json({ error: \`Missing required fields: \${missing.join(', ')}\` });\n`;
      }
